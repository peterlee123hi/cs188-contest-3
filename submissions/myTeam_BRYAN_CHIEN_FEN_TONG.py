...
class Team:
  ...
  def chooseCapsules(self, gameState, capsuleLimit, opponentEvidences):
    ...
    #                                             ['invisibility', 'speed'],
    #                                             opponentEvidences[0], None)
    # # this is the joint distribution of enemy 0 choosing invs & speed^
    # enemyAgentMarginals = []
    # for evidence in opponentEvidences:
    #     marginals = contestPowersBayesNet.computeMarginals(evidence)
    #     enemyAgentMarginals.append(marginals)
    #theirs
    # ::::: isValidPosition(self, position, isRed):
    # This function returns True if position is a valid position at which
    # a new power pellet can be placed by the team of the color specified by isRed.
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def choosePowers(self, gameState, powerLimit):
    # Goal: invis capacity and move speed if there are no lasers to blast you
    validPowers = LEGAL_POWERS[:]
    powers = util.Counter()
    powers['capacity'] = 2
    opponentPowers = self.getOpponentTeamPowers(gameState)
    if 'laser' in opponentPowers:
...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def choosePowers(self, gameState, powerLimit):
    ...
    powers = util.Counter()
    opponentPowers = self.getOpponentTeamPowers(gameState)
    if 'speed' in opponentPowers:
      powers['reflex'] = 2
      validPowers.remove('reflex')
    else:
      powers['laser'] = 2
      validPowers.remove('laser')
    for i in range(powerLimit - 2):
