...
class Team:
  def createAgents(self, firstIndex, secondIndex, isRed, gameState,
                   first = 'SpeedyOffensiveAgent', second = 'SpeedyDefensiveAgent'):
    """
...
class SpeedyOffensiveAgent(ReflexCaptureAgent):
  def choosePowers(self, gameState, powerLimit):
    powers = util.Counter()
    power_priority = ['speed', 'capacity', 'respawn', 'invisibility', 'laser']
    power_idx = 0
...
class SpeedyDefensiveAgent(ReflexCaptureAgent):
  """
  ...
  def choosePowers(self, gameState, powerLimit):
    powers = util.Counter()
    power_priority = ['laser', 'speed', 'respawn', 'capacity', 'invisibility']
    power_idx = 0
    for i in range(powerLimit):
      powerChoice = power_priority[power_idx]
      powers[powerChoice] += 1
